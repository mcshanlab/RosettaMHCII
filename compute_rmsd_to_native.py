...
def rmsd(my_pose, args):
    ...
    rms_all_movemap = rms_obj.all_atom_rmsd_residue_number(resi_list)
    rms_ca_movemap = rms_obj.CA_rmsd_residue_number(resi_list)

    return [rms_all, rms_ca, rms_all_movemap, rms_ca_movemap]
...
def write_to_file(args, tag, bind_energy, rms_all, rms_ca, rms_all_movemap, rms_ca_movemap):
    with open(args.outfile, "a") as writefilehandler:
        fcntl.flock(writefilehandler, fcntl.LOCK_EX)
        writefilehandler.write(tag+","+str(bind_energy)+","+str(rms_all)+","+str(rms_ca)+","+str(rms_all_movemap)+","+str(rms_ca_movemap)+"\n")
...
if __name__ == "__main__":
    ...
    [rms_all, rms_ca, rms_all_movemap, rms_ca_movemap] = rmsd(my_pose, args)
    bind_energy = ia(my_pose, args)
    tag = args.pdb.split(".pdb")[0]
    write_to_file(args, tag, bind_energy, rms_all, rms_ca, rms_all_movemap, rms_ca_movemap)
