...
class MHCIIALPHA(HEADER_READER):
    ...
    def get_sequence(self, header):
        if self.trim_mhc == True:
            return mhciialpha_trim[header]
        else:
            return mhciialpha[header]
    ...
    def get_headers(self):
        if super().get_headers()[0].lower != "all":
            ...
            if self.trim_mhc == True:
                return mhciialpha_trim.keys()
            else:
                return mhciialpha.keys()
